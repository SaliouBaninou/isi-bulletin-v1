...
if(!empty($_POST)){
    if(strlen($_POST['qname']) === 10){
        $dtTempe = multipleExplode(array('-', '.','/',' '), $_POST['qname']);
        $date = $dtTempe[2].'-'.$dtTempe[1].'-'.$dtTempe[0];
        $studentname = htmlentities($date." 00:00:00");
    }else{
        $studentname = " ";
        $errorCount = "La date à été mal tapé";
    }
}
$query = $pdo->prepare("SELECT * FROM students WHERE dateOfBirth = ?");
$query->execute([$studentname]);
...
<body>
    ...
    <main class="container mt-5">
        ...
        <?php endif ?>
        <?php if(!empty($errorCount)):?>
            <div class="alert alert-danger"><?=$errorCount?></div>
        <?php endif ?>
            ...
            <tbody>
                <?php foreach($students as $student):?>
                    <tr>
                        ...
                        <td style="text-align: center;"><?=$student->getField()?></td>
